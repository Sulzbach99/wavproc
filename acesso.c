...
signed char LoadAudio(audio_t *aud)
{
    ...
    fread(aud->Format, sizeof(char), 3, stdin);
    fread(aud->SubChunk1ID, sizeof(char), 4, stdin);
    ...
    fread(aud->SubChunk2ID, sizeof(char), 4, stdin);
    fread(&aud->SubChunk2Size, sizeof(int), 1, stdin);

    aud->Data = malloc(aud->ChannelNr * sizeof(int*));
    for (unsigned int k = 0; k < aud->ChannelNr; k++)
        aud->Data[k] = malloc(aud->SubChunk2Size / aud->ChannelNr);

    for (unsigned int j = 0; j < aud->SubChunk2Size / (sizeof(int) * aud->ChannelNr); j++)
